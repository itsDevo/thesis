...
def q_solver(env,verbose=0,render=False):
    ...
    if env.spec.id == "maze-sample-3x3-v0" or env.spec.id =="maze-random-3x3-v0":
        ...
        DECAY_FACTOR = 0.985  # Balanced decay for exploration
    ...
    elif env.spec.id == "maze-sample-50x50-v0" or env.spec.id =="maze-random-50x50-v0":
        LEARNING_RATE = 0.15  # Lower learning rate for stability
        DISCOUNT_FACTOR = 0.95  # High discount factor to value long-term rewards
        EPSILON = 0.7  # Moderate initial exploration rate
        DECAY_FACTOR = 0.95
        NUM_EPISODES = 5000  # Reduced episodes for time efficiency
        MIN_EXPLORE_RATE = 0.001  # Slightly higher minimum exploration
        MIN_LEARNING_RATE = 0.1  # Lower minimum learning rate for stability

    elif env.spec.id == "maze-sample-100x100-v0" or env.spec.id =="maze-random-100x100-v0":
        LEARNING_RATE = 0.2  # Lower learning rate for stability
        DISCOUNT_FACTOR = 0.99  # High discount factor to value long-term rewards
    ...
    for episode in range(NUM_EPISODES):
        ...
        if env.spec.id == "maze-sample-3x3-v0" or env.spec.id =="maze-random-3x3-v0":
            ...
            EPSILON = max(MIN_EXPLORE_RATE, min(0.7, 1.0 - 0.5 * math.log10((episode+1)/DECAY_FACTOR)))  # Slower decay for larger mazes
            LEARNING_RATE = max(MIN_LEARNING_RATE, min(0.2, 1.0 - 0.5 * math.log10((episode+1)/DECAY_FACTOR)))  # Lower learning rate for stability

        elif env.spec.id == "maze-sample-100x100-v0" or env.spec.id =="maze-random-100x100-v0":
            EPSILON = max(MIN_EXPLORE_RATE, min(0.8, 1.0 - 0.5 * math.log10((episode+1)/DECAY_FACTOR)))
            LEARNING_RATE = max(MIN_LEARNING_RATE, min(0.15, 1.0 - 0.3 * math.log10((episode+1)/DECAY_FACTOR)))
...
if __name__ == "__main__":
    env = gym.make("maze-sample-100x100-v0", enable_render=False)
